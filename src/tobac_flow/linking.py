...
def link_labels(labels1, labels2, overlap=0):
    ...
    overlap_list1 = [[j for j in np.unique(labels2.ravel()[args1[bins1[i-1]:bins1[i]]]) 
                      if j > 0 
                      and (np.count_nonzero(labels2.ravel()[args1[bins1[i-1]:bins1[i]]]==j) 
                           >= overlap * np.minimum(bins1[i]-bins1[i-1], bins2[j]-bins2[j-1]))] 
                     for i in label_list1]
    overlap_list2 = [[j for j in np.unique(labels1.ravel()[args2[bins2[i-1]:bins2[i]]]) 
                      if j > 0 
                      and (np.count_nonzero(labels1.ravel()[args2[bins2[i-1]:bins2[i]]]==j) 
                           >= overlap * np.minimum(bins2[i]-bins2[i-1], bins1[j]-bins1[j-1]))] 
                     for i in label_list2]
    
    links_list1 = []
    links_list2 = []
    while len(label_list1)>0:
        temp_links1, temp_links2 = recursive_linker([label_list1[0]], 
...
def link_dcc_cores(dcc_ds1, dcc_ds2, overlap=0):
    ...
    core_step_links1, core_step_links2 = link_labels(dcc_ds1.core_step_label.sel(t=t_overlap_list).data, 
                                                     dcc_ds2.core_step_label.sel(t=t_overlap_list).data, 
                                                     overlap=overlap)
    
    cores_list1 = [[dcc_ds1.core_step_core_index.sel(core_step=j).item() for j in i] for i in core_step_links1]
    cores_list2 = [[dcc_ds2.core_step_core_index.sel(core_step=j).item() for j in i] for i in core_step_links2]
    
    label_list1, label_list2 = np.unique(sum(cores_list1, [])).tolist(), np.unique(sum(cores_list2, [])).tolist()
    
    overlap_list1 = [list(set(sum([cores_list2[index] for index, j in enumerate(cores_list1) if i in j], []))) 
                     for i in np.unique(sum(cores_list1,[]))]
    overlap_list2 = [list(set(sum([cores_list1[index] for index, j in enumerate(cores_list2) if i in j], []))) 
...
def link_dcc_anvils(dcc_ds1, dcc_ds2, overlap=0):
    ...
    anvils_list1 = [[dcc_ds1.thick_anvil_step_anvil_index.sel(thick_anvil_step=j).item() for j in i] 
                    for i in anvil_step_links1]
    anvils_list2 = [[dcc_ds2.thick_anvil_step_anvil_index.sel(thick_anvil_step=j).item() for j in i] 
                    for i in anvil_step_links2]
    
    label_list1, label_list2 = np.unique(sum(anvils_list1, [])).tolist(), np.unique(sum(anvils_list2, [])).tolist()
    
    overlap_list1 = [list(set(sum([anvils_list2[index] for index, j in enumerate(anvils_list1) if i in j], []))) 
                     for i in label_list1]
    overlap_list2 = [list(set(sum([anvils_list1[index] for index, j in enumerate(anvils_list2) if i in j], []))) 
                     for i in label_list2]
...
class File_Linker():
    def __init__(self, files: list[pathlib.Path | str], output_func: Callable, overlap: float = 0.5) -> None:
    ...
    def core_label_map(self) -> np.ndarray:
        ...
        """
        _, _, core_links1, core_links2 = link_dcc_cores(self.current_ds, self.next_ds, overlap=self.overlap)
    ...
    def anvil_label_map(self) -> np.ndarray:
        ...
        """
        _, _, anvil_links1, anvil_links2 = link_dcc_anvils(self.current_ds, self.next_ds, overlap=0.5)
    ...
    def remap_core_labels(self, label_map) -> None:
        ...
        self.current_ds = self.current_ds.assign_coords({"core":label_map[self.current_ds.core.data]})
        ...
        self.next_ds = self.next_ds.assign_coords({"core":label_map[self.next_ds.core.data]})
...
#         print(self.current_max_core_label)
    ...
    def remap_anvil_labels(self, label_map) -> None:
        ...
        self.current_ds = self.current_ds.assign_coords({"anvil":label_map[self.current_ds.anvil.data]})
        self.next_ds = self.next_ds.assign_coords({"anvil":label_map[self.next_ds.anvil.data]})
