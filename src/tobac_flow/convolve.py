...
def convolve_same_step(
    ...
    if grid_locs is None:
        locs = (
            ...
            + np.atleast_2d(offsets)[:, np.newaxis, np.newaxis, :]
        ).astype(int)
    else:
        locs = grid_locs + np.atleast_2d(offsets)[:, np.newaxis, np.newaxis, :].astype(
            int
        )

    wh_nan = np.logical_or.reduce(
        [locs[..., 0] < 0, locs[..., 1] < 0, locs[..., 0] >= w, locs[..., 1] >= h]
    )
