...
with xr.open_dataset(dcc_files[0]) as dcc_ds:
    ...
    var_list = [var for var in dcc_ds.data_vars if dcc_ds.data_vars[var].dims in [("core_step",), ("thick_anvil_step",), ("thin_anvil_step",)]]
    var_list = ["core_edge_label_flag", "core_start_label_flag", "core_end_label_flag", 
                "thick_anvil_edge_label_flag", "thick_anvil_start_label_flag", "thick_anvil_end_label_flag", 
                "thin_anvil_edge_label_flag", "thin_anvil_start_label_flag", "thin_anvil_end_label_flag", 
                "core_nan_flag", "thick_anvil_nan_flag", "thin_anvil_nan_flag", "core_anvil_index"] + var_list
...
for f in dcc_files[1:]:
    with xr.open_dataset(f) as dcc_ds:
        ...
        if len(core_overlap) > 0:
            dataset.core_edge_label_flag.loc[core_overlap].data = np.logical_or(dataset.core_edge_label_flag.loc[core_overlap].data, dcc_ds.core_edge_label_flag.loc[core_overlap].data)
            dataset.core_end_label_flag.loc[core_overlap].data = dcc_ds.core_end_label_flag.loc[core_overlap].data
            dataset.core_nan_flag.loc[core_overlap].data = np.logical_or(dataset.core_nan_flag.loc[core_overlap].data, 
            ...
            dataset.core_anvil_index.loc[wh_anvil_is_zero_cores].data = dcc_ds.core_anvil_index.loc[wh_anvil_is_zero_cores].data
        
        anvil_overlap = sorted(list(set(dataset.anvil.data) & set(dcc_ds.anvil.data)))
        if len(core_overlap) > 0:
            dataset.thick_anvil_edge_label_flag.loc[anvil_overlap].data = np.logical_or(dataset.thick_anvil_edge_label_flag.loc[anvil_overlap].data, dcc_ds.thick_anvil_edge_label_flag.loc[anvil_overlap].data)
            dataset.thick_anvil_end_label_flag.loc[anvil_overlap].data = dcc_ds.thick_anvil_end_label_flag.loc[anvil_overlap].data
            dataset.thick_anvil_nan_flag.loc[anvil_overlap].data = np.logical_or(dataset.thick_anvil_nan_flag.loc[anvil_overlap].data, 
                                                                                dcc_ds.thick_anvil_nan_flag.loc[anvil_overlap].data)
            dataset.thin_anvil_edge_label_flag.loc[anvil_overlap].data = np.logical_or(dataset.thin_anvil_edge_label_flag.loc[anvil_overlap].data, dcc_ds.thin_anvil_edge_label_flag.loc[anvil_overlap].data)
            dataset.thin_anvil_end_label_flag.loc[anvil_overlap].data = dcc_ds.thin_anvil_end_label_flag.loc[anvil_overlap].data
            dataset.thin_anvil_nan_flag.loc[anvil_overlap].data = np.logical_or(dataset.thin_anvil_nan_flag.loc[anvil_overlap].data, 
        ...
        dataset = xr.combine_by_coords(data_objects=[dataset, dcc_ds.sel(core=core_different, 
                                                                         anvil=anvil_different)], 
                                       data_vars="different", coords="different", join="outer")
...
thin_anvil_any_nan_step = dataset.thin_anvil_step_BT_mean.groupby(dataset.thin_anvil_step_anvil_index).reduce(any_nan)

def start_end_diff(x, *args, **kwargs):
    return x[-1] - x[0]

anvil_lifetime = dataset.thick_anvil_step_t.groupby(dataset.thick_anvil_step_anvil_index).reduce(start_end_diff)

anvil_invalid_lifetime = anvil_lifetime < np.timedelta64(timedelta(minutes=15))

anvil_nan_flag = np.logical_or.reduce([dataset.thick_anvil_nan_flag.data, 
...
core_invalid_anvil = np.logical_not(np.isin(dataset.core_anvil_index.data, dataset.anvil.data))

def start_end_diff(x, *args, **kwargs):
    return x[0] - x[-1]

core_bt_change = dataset.core_step_BT_mean.groupby(dataset.core_step_core_index).reduce(start_end_diff)
...
core_max_time_diff = dataset.core_step_t.groupby(dataset.core_step_core_index).reduce(max_t_diff)
...
core_lifetime = dataset.core_step_t.groupby(dataset.core_step_core_index).reduce(end_start_diff)

core_invalid_lifetime = core_lifetime < np.timedelta64(timedelta(minutes=15))

wh_core_invalid = np.logical_or.reduce([dataset.core_start_label_flag.data, 
...
anvil_initial_core = dataset.core.groupby(dataset.core_anvil_index).min().rename(core_anvil_index="anvil")

core_end_step = dataset.core_step.groupby(dataset.core_step_core_index).max().rename(core_step_core_index="core")

core_end_t = dataset.core_step_t.loc[core_end_step]

thick_anvil_max_area_step = [int(group[1].idxmax().item()) for group in dataset.thick_anvil_step_area.groupby(dataset.thick_anvil_step_anvil_index)]
...
core_start_step = dataset.core_step.groupby(dataset.core_step_core_index).min().rename(core_step_core_index="core")
...
def weighted_average_groupby(field, area, groups, coord):
    return xr.DataArray(
        [np.average(field_group[1], weights=area_group[1]) 
         for field_group, area_group in zip(field.groupby(groups), area.groupby(groups))], 
        {coord.name:coord}
    )
...
def argmax_groupby(field, find_max, groups, coord):
    return xr.DataArray(
        [field_group[1].data[np.argmax(max_group[1].data)] 
         for field_group, max_group in zip(field.groupby(groups), find_max.groupby(groups))], 
        {coord.name:coord}
    )
...
def argmin_groupby(field, find_min, groups, coord):
    return xr.DataArray(
        [field_group[1].data[np.argmax(min_group[1].data)] 
         for field_group, min_group in zip(field.groupby(groups), find_min.groupby(groups))], 
        {coord.name:coord}
    )
...
def calc_max_cooling_rate(step_bt, step_t):
    ...
    if len(step_bt) >=4:
        ...
        step_bt_diff = (step_bt[0] - step_bt[-1]) / ((step_t[0] - step_t[-1]).astype("timedelta64[s]").astype("int")/60)
    return step_bt_diff
...
def calc_combined_std(step_std, step_mean, step_area):
    ...
    return ((np.sum(step_area * step_std) + np.sum(step_area * (step_mean - combined_mean)**2)) / np.sum(step_area))**0.5

def combined_mean_groupby(means, area, groups, coord):
    return xr.DataArray(
        [calc_combined_mean(means_group[1].data, area_group[1].data) 
         for means_group, area_group in zip(means.groupby(groups), area.groupby(groups))], 
        {coord.name:coord}
    )

def combined_std_groupby(stds, means, area, groups, coord):
    return xr.DataArray(
        [calc_combined_std(stds_group[1].data, means_group[1].data, area_group[1].data) 
         for stds_group, means_group, area_group 
         in zip(stds.groupby(groups), means.groupby(groups), area.groupby(groups))], 
        {coord.name:coord}
    )
...
thick_anvil_start_step = dataset.thick_anvil_step.groupby(dataset.thick_anvil_step_anvil_index).min().rename(thick_anvil_step_anvil_index="anvil")
...
dataset["thick_anvil_start_lat"] = dataset.thick_anvil_step_lat.loc[thick_anvil_start_step]
dataset["thick_anvil_start_lon"] = dataset.thick_anvil_step_lon.loc[thick_anvil_start_step]

dataset["thick_anvil_start_t"] = dataset.thick_anvil_step_t.loc[thick_anvil_start_step]

thick_anvil_end_step = dataset.thick_anvil_step.groupby(dataset.thick_anvil_step_anvil_index).max().rename(thick_anvil_step_anvil_index="anvil")
...
dataset["thick_anvil_lifetime"] = dataset.thick_anvil_end_t - dataset.thick_anvil_start_t
...
dataset["thick_anvil_total_area"] = xr.DataArray(
    dataset.thick_anvil_step_area.groupby(dataset.thick_anvil_step_anvil_index).sum().data, 
    {"anvil":dataset.anvil}
)

dataset["thick_anvil_max_area"] = xr.DataArray(
    dataset.thick_anvil_step_area.groupby(dataset.thick_anvil_step_anvil_index).max().data, 
    {"anvil":dataset.anvil}
)
...
thin_anvil_start_step = dataset.thin_anvil_step.groupby(dataset.thin_anvil_step_anvil_index).min().rename(thin_anvil_step_anvil_index="anvil")
...
thin_anvil_end_step = dataset.thin_anvil_step.groupby(dataset.thin_anvil_step_anvil_index).max().rename(thin_anvil_step_anvil_index="anvil")
...
dataset["thin_anvil_total_area"] = xr.DataArray(
    dataset.thin_anvil_step_area.groupby(dataset.thin_anvil_step_anvil_index).sum().data, 
    {"anvil":dataset.anvil}
)

dataset["thin_anvil_max_area"] = xr.DataArray(
    dataset.thin_anvil_step_area.groupby(dataset.thin_anvil_step_anvil_index).max().data, 
    {"anvil":dataset.anvil}
)
...
dataset["anvil_initial_core_index"] = xr.DataArray(
    dataset.core.groupby(dataset.core_anvil_index).min().data, 
    {"anvil":dataset.anvil}
)
