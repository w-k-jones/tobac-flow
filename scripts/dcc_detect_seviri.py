...
parser.add_argument('-fd', help='SEVIRI file directory',
                    default='/gws/nopw/j04/eo_shared_data_vol1/satellite/seviri-orac/', type=str)
...
def main(start_date, end_date, x0, x1, y0, y1, save_path, seviri_data_path):
    ...
    cld_files = find_seviri_files(start_date, end_date, n_pad_files=t_offset+1, file_type="cloud",
                                  file_path=os.path.join(seviri_data_path, "cld"))

    cld_ds = xr.open_mfdataset(cld_files, combine="nested", concat_dim="t").isel(across_track=slice(x0,x1),
    ...
    flx_ds = xr.open_mfdataset(flx_files, combine="nested", concat_dim="t").isel(across_track=slice(x0,x1),
                                                                                 along_track=slice(y0,y1))
    ...
    toa_swup_cre = create_dataarray(flx_ds.toa_swup.data-flx_ds.toa_swup_clr, flx_ds.dims, "toa_swup_cre", units="")
    toa_lwup_cre = create_dataarray(flx_ds.toa_lwup.data-flx_ds.toa_lwup_clr, flx_ds.dims, "toa_lwup_cre", units="")

    for field in (flx_ds.toa_swdn, flx_ds.toa_swup, flx_ds.toa_lwup, toa_net,
                  toa_swup_cre, toa_lwup_cre, toa_cre):
        [add_dataarray_to_ds(da, dataset) for da in weighted_statistics_on_labels(dataset.core_label,
                                                                                  field.compute(),
                                                                                  area_stack,
                                                                                  name='core',
