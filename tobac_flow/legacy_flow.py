...
class Flow_Func(object):
    ...
    def __getitem__(self, items):
        ...
        """
        return Flow_Func(
            self.flow_x_for[items],
            self.flow_x_back[items],
            self.flow_y_for[items],
            self.flow_y_back[items],
        )
    ...
    def __call__(self, t):
        ...
        """
        return (
            0.5 * t * (t + 1) * self.flow_x_for + 0.5 * t * (t - 1) * self.flow_x_back,
...
def _checkstruct(structure, n_dims):
    ...
    if hasattr(structure, "shape"):
        ...
        for s in structure.shape:
            if s not in [1, 3]:
                raise ValueError(
                    "structure input must be an array with dimensions of length 1 or 3"
                )
        if len(structure.shape) < n_dims:
...
def _gen_flow_ravel_inds(flow_func, structure, wrap=False):
    ...
    structure_offsets = [
        arr.reshape((-1,) + (1,) * (n_dims - 1)) - 1 for arr in np.where(structure != 0)
    ]
    ...
    shape_ranges = [
        np.arange(s).reshape(np.roll((-1,) + (1,) * (n_dims - 2), i))
    ...
    for t in range(shape[0]):
        ...
        temp_inds[0] = (
            flow_inds[0]
            + np.round(flow_func.flow_y_for[t]).astype(int) * whp1
            + np.round(flow_func.flow_y_back[t]).astype(int) * whm1
        )
        temp_inds[1] = (
            flow_inds[1]
            + np.round(flow_func.flow_x_for[t]).astype(int) * whp1
            + np.round(flow_func.flow_x_back[t]).astype(int) * whm1
        )

        ravelled_index = np.ravel_multi_index(
            [
                (structure_offsets[0] + t) % shape[0],
                temp_inds[0] % shape[1],
                temp_inds[1] % shape[2],
            ],
            shape,
        ...
        if wrap:
            ...
            mask = sum(
                [
                    (structure_offsets[0] + t) % shape[0] != (structure_offsets[0] + t),
...
def flow_convolve_nearest(
    ...
    **kwargs
    ...
    for t in range(data.shape[0]):
        ...
        temp = ma.array(
            data.ravel()[ravelled_index].reshape((n_elements,) + data.shape[1:])
            * structure_factor,
            mask=mask,
            dtype=data.dtype,
        )
...
def flow_argmin_nearest(data, argmin, flow_func, structure=None, dtype=None):
    ...
    shape_arrays = np.meshgrid(
        *(np.arange(s, dtype=int) for s in argmin.shape[1:]), indexing="ij"
    )
...
def flow_local_min(flow_stack, structure=None, ignore_nan=False):
    ...
    if ignore_nan:
        return (
            flow_convolve(flow_stack, structure=structure, function=np.nanmin)
            == flow_stack[1]
        )
    else:
        return (
            flow_convolve(flow_stack, structure=structure, function=np.min)
            == flow_stack[1]
        )
        ...
        if magnitude:
            ...
            def temp_sobel_func(temp, ax, counter=[0]):
                ...
                for i in axis:
                    sobel_matrix = (
                        np.transpose(
                            get_sobel_matrix(3), np.roll(np.arange(3), (1 + i) % 3)
                        )
                        .ravel()
                        .reshape((-1, 1, 1))
                        .astype(temp.dtype)
                    ...
                    output += (
                        np.sum((temp - flow_stack[1][counter[0]]) * sobel_matrix, 0)
                        ** 2
                    )
                counter[0] += 1
                return output**0.5

            output = flow_convolve(
                flow_stack, structure=np.ones([3, 3, 3]), function=temp_sobel_func
            )
        else:
            for i in axis:
                ...
                def temp_sobel_func(temp, axis, counter=[0]):
                    ...
                    output = np.sum(
                        (temp - flow_stack[1][counter[0]]) * sobel_matrix, axis
                    )
                    counter[0] += 1
                    return output
        ...
        if magnitude:
            ...
            def temp_sobel_func(temp, ax, counter=[0]):
                ...
                for i in axis:
                    ...
                    output += (
                        np.sum(
                            np.maximum(temp - flow_stack[1][counter[0]], 0)
                            * sobel_matrix,
                            0,
                        )
                        ** 2
            ...
            for i in axis:
                ...
                def temp_sobel_func(temp, axis, counter=[0]):
                    ...
                    output = np.sum(
                        np.maximum(temp - flow_stack[1][counter[0]], 0) * sobel_matrix,
                        axis,
                    )
                    counter[0] += 1
                    return output
        ...
        if magnitude:
            ...
            def temp_sobel_func(temp, ax, counter=[0]):
                ...
                for i in axis:
                    ...
                    output += (
                        np.sum(
                            np.minimum(temp - flow_stack[1][counter[0]], 0)
                            * sobel_matrix,
                            0,
                        )
                        ** 2
            ...
            for i in axis:
                ...
                def temp_sobel_func(temp, axis, counter=[0]):
                    ...
                    output = np.sum(
                        np.minimum(temp - flow_stack[1][counter[0]], 0) * sobel_matrix,
                        axis,
                    )
                    counter[0] += 1
                    return output
...
def flow_network_watershed(
    ...
    min_convolve = flow_convolve_nearest(
        field, flow_func, structure=structure, function=ma.argmin, dtype=np.uint8
    )
    min_convolve = np.minimum(
        np.maximum(min_convolve, 0), np.sum(structure != 0).astype(np.uint8) - 1
    )
    inds_neighbour = flow_argmin_nearest(
        inds, min_convolve, flow_func, structure=structure, dtype=inds_dtype
    )
    del min_convolve
    ...
    if hasattr(inds_neighbour, "mask"):
        wh = np.logical_or(
            np.logical_or(inds_neighbour.data < 0, inds_neighbour.data > inds.max()),
            inds_neighbour.mask,
        )
        if np.any(wh):
    ...
    for i in range(max_iter):
        inds_neighbour[wh_to_fill] = inds_neighbour.ravel()[
            inds_neighbour[wh_to_fill].ravel()
        ]
    ...
    for iter in range(1, max_iter + 1):
        ...
        def fill_mask_argmin(temp, axis):
            ...
            temp_fill = ma.array(
                fill.ravel()[ravelled_index].reshape(temp.shape),
                mask=mask,
                dtype=fill.dtype,
            )
            temp.mask = np.logical_or(temp.mask, fill[t] == temp_fill)
            return ma.array(
                np.argmin(temp, axis), mask=np.all(temp.mask, axis), dtype=np.uint8
            )

        argmin_edge = flow_convolve_nearest(
            field,
            flow_func,
        ...
        min_edge = flow_argmin_nearest(
            field, argmin_edge.filled(fill_value=0), flow_func, structure=new_struct
        )
        min_edge = ma.array(min_edge, mask=argmin_edge.mask)
        inds_edge = flow_argmin_nearest(
            inds,
            argmin_edge.filled(fill_value=0),
            flow_func,
            structure=new_struct,
            dtype=inds_dtype,
        )
            ...
            if wh.size > 0:
                if np.all(min_edge.mask.ravel()[wh]):
                    ...
                    output = fill.ravel()[
                        inds_edge.ravel()[wh][
                            np.nanargmin(
                                np.maximum(min_edge.ravel()[wh], field.ravel()[wh])
                            )
                        ]
                    ]
            else:
        ...
        new_label = ma.array(
            list(range(max_markers + 1))
            + [
                get_new_label(k) if region_bins[k] > 0 else 0
...
def flow_label(data, flow, structure=ndi.generate_binary_structure(3, 1)):
    ...
    t_labels = ndi.label(
        data, structure=structure * np.array([0, 1, 0])[:, np.newaxis, np.newaxis]
    )[0].astype(float)
    ...
    if np.any(structure * np.array([1, 0, 0])[:, np.newaxis, np.newaxis]):
        p_labels = flow_convolve_nearest(
            t_labels,
            flow,
    ...
    if np.any(structure * np.array([0, 0, 1])[:, np.newaxis, np.newaxis]):
        n_labels = flow_convolve_nearest(
            t_labels,
            flow,
        ...
        for k in sorted(list(n_label_map.keys()))[::-1]:
            ...
            while n_label_map[k] != n_label_map[n_label_map[k]]:
                ...
                if n_label_map[n_label_map[k]] in prev_labels:
                    n_label_map[k] = max(
                        prev_labels[prev_labels.index(n_label_map[n_label_map[k]]) :]
                    )
                    break
