...
def get_marker_distance_cylinder(markers, time_margin):
    ...
    for i in range(markers.shape[0]):
        i_slice = slice(np.maximum(i-time_margin,0),np.minimum(i+time_margin+1, markers.shape[0]))
        ...
        closest_markers2[i] = np.take_along_axis(closest_markers[i_slice], argmin, axis=0)
