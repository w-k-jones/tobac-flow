...
def get_step_labels_for_label(labels, step_labels,
...
#     Get flat (2d) labels
    if subsegment_shrink == 0:
        ...
        flat_labels = subsegment_labels(mask.astype(bool),
    ...
    for label in range(1, bins.size):
        if not processed_labels[label]:
            ...
            while i < len(label_map[label]):
                find_neighbour_labels(label_map[label][i], label_map[label],
                                      bins, args, processed_labels,
